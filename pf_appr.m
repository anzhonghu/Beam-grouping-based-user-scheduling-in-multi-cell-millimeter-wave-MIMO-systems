...
for l1 = 1 : L
    for k = 1 : K
        ...
        nn = ny * naz + nx + 1;
        Ul = U(:, nn);
        hll = wj' * H(:,(l1-1)*K*L*n_arr+(l1-1)*K*n_arr+(k-1)*n_arr+1:(l1-1)*K*L*n_arr+(l1-1)*K*n_arr+k*n_arr) * Ul;
        capa_current_s(k, l1) = log2(1 + abs(hll)^2 * Pvsigma2);
    end
end
...
for tfr = 1 : time_fre_resources
    ...
    K_ind = zeros(Nr, L);
    for l1 = 1 : L
        [~, Csel] = sort(capa_current_s(:,l1) ./ capacity_cumu(:, l1), 'descend');
        kc = 0;
        for n = 1 : K
            if 1==n
                k = Csel(n,1);
